...
class bert_Transformer(nn.Module):
    def __init__(self, local_model_path=None):
        super().__init__()
        # 首先尝试从本地加载模型，如果本地路径提供了的话
        # 否则尝试从Hugging Face下载（可能会失败，如果网络不可用）
        try:
            if local_model_path:
                print(111)
                ...
                # 尝试从默认路径加载，如果存在的话
                default_local_path = "./checkpoints/bert-base-uncased"
                print(222, os.path.exists(default_local_path))
                if os.path.exists(default_local_path):
                    self.model = BertModel.from_pretrained(default_local_path)
                else:
                    self.model = BertModel.from_pretrained("bert-base-uncased")
        except Exception as e:
            print(f"警告: 无法加载BERT模型: {e}")
            print("请确保您已经下载了bert-base-uncased模型并放在正确的位置")
            print("您可以手动从Hugging Face下载模型并放置在 ./checkpoints/bert-base-uncased/")
            raise
