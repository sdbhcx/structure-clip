...
class FlavaWrapper:
    def __init__(self, root_dir, device):
        self.model = FlavaForPreTraining.from_pretrained("./checkpoints/facebook/flava-full", cache_dir=root_dir).eval().to(device)
        self.feature_extractor = FlavaFeatureExtractor.from_pretrained("./checkpoints/facebook/flava-full", cache_dir=root_dir)
        self.tokenizer = BertTokenizer.from_pretrained("./checkpoints/facebook/flava-full", cache_dir=root_dir)
        self.processor = FlavaProcessor.from_pretrained("./checkpoints/facebook/flava-full", cache_dir=root_dir)
        self.device = device
